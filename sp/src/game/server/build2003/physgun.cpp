...
void CWeaponGravityGun::EffectUpdate(void)
{
	...
	if (m_hObject == NULL && tr.DidHitNonWorldEntity())
	{
		...
		ClearMultiDamage();
		pEntity->DispatchTraceAttack(CTakeDamageInfo(pOwner, pOwner, 0, DMG_PHYSICSGUN), forward, &tr);
		ApplyMultiDamage();
	...
	if (pObject)
	{
		...
		pObject->TakeDamage(CTakeDamageInfo(this, pOwner, 0, DMG_PHYSICSGUN));
