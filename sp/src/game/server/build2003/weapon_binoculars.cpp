...
//-----------------------------------------------------------------------------
static int g_nZoomFOV[6] = { 0, 40, 30, 20, 10, 5 };
...
void CWeaponBinoculars::CacheSoundSources(void)
{
	...
	while (pEnt)
	{
		if (&CBaseEntity::GetSoundEmissionOrigin) //is sound source
		{
			*pSources = pEnt;
			pSources++;
			count++;
...
void CWeaponBinoculars::Precache(void)
{
	...
	//PrecacheSound (pStaticSounds); temp fix is disabling sounds
	//PrecacheSound (pSignalSounds); 
	PrecacheScriptSound("Weapon_binoculars.Special3");
	PrecacheScriptSound("Weapon_binoculars.Special4");
