...
public:
	...
	CNetworkVar(float, m_flTimeBurnOut);	// when will the flare burn out? Custom
	...
	bool		m_bFading;
	CNetworkVar(bool, m_bLight); // We have light (Making it like a fact)
	CNetworkVar(bool, m_bSmoke); // We have smoke (Making it like a fact)
