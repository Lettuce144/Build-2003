...
{
	return(CLASS_COMBINE); //Fix me! Add an actual class
}
...
void CNPC_WScanner::PlayAlarmSound(void)
{
	...
	if (gpGlobals->curtime > m_fNextAlarmSoundTime)
	{
		...
		Vector vHintPos;
		GetHintNode()->GetPosition(this, &vHintPos); //Edit
...
void CNPC_WScanner::Precache(void)
{
	...
	PrecacheScriptSound("NPC_WScanner.HoverAlarm");
	PrecacheScriptSound("NPC_WScanner.Hover");
	PrecacheScriptSound("NPC_WScanner.Alarm");
	PrecacheScriptSound("NPC_WScanner.Split");
	PrecacheScriptSound("NPC_WScanner.Attach");
	PrecacheScriptSound("NPC_WScanner.LaserWarmUp");
	PrecacheScriptSound("NPC_WScanner.LaserShoot");
...
void CNPC_WScanner::RunTask(const Task_t *pTask)
{
	...
	// If my enemy has moved significantly, update my path //Edit
...
void CNPC_WScanner::StartTask(const Task_t *pTask)
{
	...
	case TASK_FIND_HINTNODE:
	{
		if (!GetHintNode())
		{
			CAI_HintManager::FindHint(this, HINT_NONE, pTask->flTaskData, 5000);
		}
		if (GetHintNode())
		{
			TaskComplete();
