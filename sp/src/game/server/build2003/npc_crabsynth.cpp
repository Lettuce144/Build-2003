...
void CCrabSynth::InitCustomSchedules(void)
{
	...
	AI_LOAD_SCHEDULE(CCrabSynth, SCHED_CRAB_CHASE_ENEMY);
}
...
int CCrabSynth::OnTakeDamage_Alive(const CTakeDamageInfo& inputInfo)
{
	...
	if (info.GetDamageType() & DMG_BLAST)
	{
		if (info.GetInflictor())
		{
			...
			if (info.GetInflictor()->GetAbsOrigin().z - GetAbsOrigin().z < ((WorldSpaceCenter().z - WorldSpaceCenter().z) * 0.5f))
			{
