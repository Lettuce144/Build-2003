...
//
// Purpose: Ouch, that is hot
// 
// Todo: Make a particle effect, and use smg as place holder
//		 (As in a asset)^
//
// $NoKeywords: $
//=============================================================================//
...
ConVar sk_plr_dmg_flamethrower("sk_plr_dmg_flamethrower", "3", FCVAR_REPLICATED);
ConVar sk_flamethrower_fire_stream("sk_flamethrower_fire_stream", "1", FCVAR_REPLICATED);
ConVar sk_flamethrower_burn_seconds("sk_flamethrower_burn_seconds", "3");

//-----------------------------------------------------------------------------
// Crossbow Bolt
//-----------------------------------------------------------------------------
class CFlamethrowerFireBall : public CBaseCombatCharacter
{
	DECLARE_CLASS(CFlamethrowerFireBall, CBaseCombatCharacter);
...
LINK_ENTITY_TO_CLASS(flamethrower_fire_ball, CFlamethrowerFireBall);

BEGIN_DATADESC(CFlamethrowerFireBall)
// Function Pointers,
...
void CWeaponFlamethrower::StartImmolating()
{
...
void CWeaponFlamethrower::StopImmolating()
{
	CBasePlayer* pOwner = ToBasePlayer(GetOwner());
	m_flBurnRadius = 0.0;
	SetThink(NULL);
	m_vecFlamethrowerTarget = IMMOLATOR_TARGET_INVALID;
...
void CWeaponFlamethrower::StopImmolatingSilent()
{
	CBasePlayer* pOwner = ToBasePlayer(GetOwner());
	m_flBurnRadius = 0.0;
	SetThink(NULL);
...
int CWeaponFlamethrower::WeaponRangeAttack1Condition(float flDot, float flDist)
{
	...
	if (m_vecFlamethrowerTarget == IMMOLATOR_TARGET_INVALID)
	{
		// No target!
		return COND_NONE;
	}
...
{
	if (sk_flamethrower_fire_stream.GetBool())
	{
		CBasePlayer* pOwner = ToBasePlayer(GetOwner());
		...
		CFlamethrowerFireBall* pPlasmaBall = (CFlamethrowerFireBall*)CBaseEntity::Create("flamethrower_fire_ball", muzzlePoint, vecAngles, pOwner);
		...
		pPlasmaBall->SetContextThink(&CFlamethrowerFireBall::SUB_Remove, gpGlobals->curtime + 0.75, "KillBoltThink");
	...
	// Calculate damage
	CTakeDamageInfo info = CTakeDamageInfo(this, this, sk_plr_dmg_flamethrower.GetFloat(), DMG_BURN | DMG_DIRECT);
...
void CWeaponFlamethrower::ImmolationDamage(const CTakeDamageInfo& info, const Vector& vecSrcIn, float flRadius, int iClassIgnore)
{
...
// 1upD
bool CWeaponFlamethrower::ImmolateEntity(const CTakeDamageInfo& info, CBaseCombatCharacter* pBCC, int iClassIgnore)
{
	if (pBCC)
	{
...
//-----------------------------------------------------------------------------
bool CFlamethrowerFireBall::CreateVPhysics(void)
{
...
//-----------------------------------------------------------------------------
unsigned int CFlamethrowerFireBall::PhysicsSolidMaskForEntity() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CFlamethrowerFireBall::Spawn(void)
{
	Precache();
	...
	SetTouch(&CFlamethrowerFireBall::BoltTouch);
}


void CFlamethrowerFireBall::Precache(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CFlamethrowerFireBall::BoltTouch(CBaseEntity* pOther)
{
	Assert(pOther);
	if (!pOther->IsSolid())
		return;
...
void CFlamethrowerFireBall::IgniteOtherIfAllowed(CBaseEntity* pOther)
{
	...
	if (pNPC) {
		...
		// Burn this NPC
		pNPC->IgniteLifetime(sk_flamethrower_burn_seconds.GetInt());
	}
	...
	if (pBreakable)
	{
		pBreakable->IgniteLifetime(sk_flamethrower_burn_seconds.GetInt());
	...
	// Do damage
	pOther->TakeDamage(CTakeDamageInfo(this, this, sk_plr_dmg_flamethrower.GetFloat(), (DMG_BULLET | DMG_BURN)));
